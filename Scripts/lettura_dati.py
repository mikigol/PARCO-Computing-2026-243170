...
for matrix in matrices:
    ...
    if len(df_seq) > 0:
        ...
        print(f"\nSequential: {seq_time:.6f}s → {seq_gflops:.3f} GFLOP/s")
        ...
        if len(df_sched) > 0:
            ...
            for num_t in thread_counts_gflops:
                ...
                if len(df_config) > 0:
                    ...
                    print(f"  Threads={num_t}: {time_val:.6f}s → {gflops_val:.3f} GFLOP/s")
...
print("\n" + "=" * 80)
print("CREAZIONE GRAFICO 4: GFLOP/s (3 SUBPLOT PER SCHEDULE)")
print("=" * 80)
...
for idx, schedule in enumerate(schedules):
    ...
    # Limite Y uniforme
    max_gflops = gflops_df['GFLOP/s'].max()
    ax.set_ylim(bottom=0, top=max_gflops * 1.15)

# Titolo generale
fig4.suptitle('GFLOP/s Performance: Sequential vs Parallel (8, 16, 32 threads)\nEach schedule uses its optimal chunk_size per matrix',
...
print(f"GFLOP/s massimo: {gflops_df['GFLOP/s'].max():.3f} ({gflops_df.loc[gflops_df['GFLOP/s'].idxmax(), 'matrix']} - {gflops_df.loc[gflops_df['GFLOP/s'].idxmax(), 'schedule']} - {gflops_df.loc[gflops_df['GFLOP/s'].idxmax(), 'num_threads']} threads)")
